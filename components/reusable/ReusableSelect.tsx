  ...
  "flex items-center justify-between transition-all duration-200",
  {
    variants: {
      variant: {
        ...
        primary: "min-w-[180px] h-10 md:h-15 justify-center gap-2 px-4 md:px-6 py-3 md:py-4 md:rounded-2xl rounded-xl text-[#070707] font-medium text-base bg-[linear-gradient(136deg,#FFBB1C_0%,#E28611_100%)] hover:opacity-90 active:scale-[0.98] hover:shadow-lg hover:shadow-orange-500/20 border-none [&>svg]:text-[#070707]",
      },
    },
    defaultVariants: {
      variant: "default",
...
export function ReusableSelect({
  ...
  return (
    ...
    >
      ...
      <SelectContent align="end" className={selectContentVariants({ variant })}>
        {options.map((opt) => (
          <SelectItem 
            key={opt.value} 
            value={opt.value}
